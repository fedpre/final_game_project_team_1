import arcade
class DoUpdates:
    def __init__(self, player, physics_engine, camera, score, timer, enemies, power_engine):
        self._player = player
    ...
    def check_flag_collision(self, object_list, setup, levels, gameover):
        object_hit_list = arcade.check_for_collision_with_list(
            self._player, object_list
        )
    ...
    def update_enemies(self, enemy_list):
        for enemy in enemy_list:
            self._power_engine.set_velocity(enemy, (-150, 0))
            
        self._power_engine.step()
        self._power_engine.resync_sprites()
